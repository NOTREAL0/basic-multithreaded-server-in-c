...
#include <sys/socket.h> //socket() bind() listen() accept() recv() send() socklen_t ssize_t size_t
...
handler(void* arg)
{
    ...
    sockfd = *(int*) arg;
    free(arg);
    //the below operations are for testing, in reality they need to be replaced by a handler
